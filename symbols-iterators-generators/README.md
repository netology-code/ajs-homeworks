...
Вам необходимо написать итератор, который начинает перебор с самого слабого из персонажей противника (самый слабый = защита + уровень здоровья), учитывая его атакующие способности (дальность атаки и урон - используйте формулу: атакующие способности = дальность атаки + урон). Т.е. если у вас три персонажа:
...
То компьютер должен атаковать первого персонажа, т.к.:
...
Т.е. если брать данные из предыдущей задачи, то атакуется сначала третий, потом первый, потом второй.
