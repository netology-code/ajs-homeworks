...
Класс-заглушка для `GameSavingData`:
```javascript
...
Функция-заглушка для `readGameSaving`:
```javascript
...
Используя механизмы Jest, замокайте функции-заглушку `readGameSaving`, чтобы у вас была возможность протестировать как `resolve`, так и `reject` на выходе из этой функции.
